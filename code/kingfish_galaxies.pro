function kingfish_galaxies, name $
                          , data_dir=data_dir $
                          , found=found $
                          , quiet=quiet
;+
...
; PURPOSE:
;
; Retreives structure and other information for KINGFISH galaxies.  Adapted
; very faithfully from things_galaxies, but with different datafiles to 
; conform to the KINGFISH sample info.
;
; CATEGORY:
;
; Science helper?
...
; OPTIONAL INPUTS:
;
; data_dir : the location of the KINGFISH structure files
;
; KEYWORD PARAMETERS:
;
; none
...
;
; blantantly ripped off by KMS - 12/17/11
;-
...
;    data_dir = '$MY_IDL/things/gal_data/'
	data_dir = '/home/leroy/sfgal/code/kingfish_gal_data/'
endif
...
distfile = data_dir+'kf_distances.dat'
readcol, distfile, dist_name, dist, format='A,F', comment='#', /silent
ind = where(strlowcase(dist_name) eq name, ct)
if ct eq 0 then begin
   for i = 0, n_elements(alias)-1 do begin
      if ct gt 0 then continue
      ind = where(strlowcase(dist_name) eq alias[i], ct)
   endfor
endif
if ct gt 0 then begin
   s.dist_mpc = dist[ind]
...
;readcol, kinfile, delim='|', skip = 1 $
;  , kin_name, kin_vflat, kin_lflat_deg, kin_lflat_kpc $
...
;if ct gt 0 then begin
;   s.vhel_kms = radial_velocity[hi_ind]
;   s.vrot_kms = rotation_velocity[hi_ind]
;endif
